public class Integrator {
    static func integrate(_ values: [Point]) -> Double {
        
        var outputData: [Point] = []
        
        for (index, thisPoint) in values[0..<values.count - 1].enumerated() {
            let nextPoint = values[index + 1]
            
            print("\(thisPoint) to \(nextPoint)")
            ...
            outputData.append(Point(newX, totalArea))
        }
        
        // Sum up all of the values to get our final answer
        return outputData.reduce(0.0) { soFar, point in
            soFar + point.y
        }
    }
}
